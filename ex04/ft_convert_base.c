...
//     printf("CONVERSION\n\n");
//     printf("Input: %s\nBase From: %s\nBase To: %s\nResult: %s\n", input, base_from, base_to, ft_convert_base(input, base_from, base_to));
...
// 				if (!(chkbse_getsz(bases[j]) <= 1 || chkbse_getsz(bases[k]) <= 1) && value != 0)
// 				{
// 					char	*nbr_converted;
// 					nbr_converted = ft_putnbr_base(value, bases[k]);
// 					//printf("\tbase_from[%d]\tinput[%d]\tbase_to[%d]\n", i, j, k);
// 					printf("-------------------------------------------------------------------------------\n{%s}\n\t\t\t> %s \n\t\t\t\t\t\t<%ld>\n{%s}\n\t\t\t> %s\n\n", bases[j], inputs[i], value, bases[k], nbr_converted);
// 				}
// 				else
// 				{
// 					// printf("KO |%ld| : bases[%d](%d) : %s, inputs[%d], bases[%d](%d) : %s\n",
// 					// 	value, j,chkbse_getsz(bases[j]), bases[j], i, k, chkbse_getsz(bases[k]), bases[k]);
// 				}
// 				k++;
// 			}
// 			j++;
...
// 			a = 0;
// 			while (a < 5)
// 				printf("%s\t\t", ft_convert_base(inputs[i], bases[a++], bases[b]));
// 			printf("\n");
// 			b++;
// 		}
// 		i++;
